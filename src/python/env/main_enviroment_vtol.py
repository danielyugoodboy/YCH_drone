...
class Drone_Enviroment():
    def __init__(self, drone_name=""):
        ...
        print("[State] : Wait for vtol_transition service")
        rospy.wait_for_service(self.name+"/mavros/cmd/vtol_transition")  # wait service
        self.set_vtol_client = rospy.ServiceProxy(self.name+"/mavros/cmd/vtol_transition", CommandVtolTransition)
        ...
        print("[State] : Set vtol CommandBoolRequest")
        self.vtol_FWcmd = CommandVtolTransitionRequest()
        self.vtol_FWcmd.state = CommandVtolTransitionRequest.STATE_FW
        self.vtol_MCcmd = CommandVtolTransitionRequest()
        self.vtol_MCcmd.state = CommandVtolTransitionRequest.STATE_MC
    ...
    def position_step(self, action):
        ...
        try:
            response = self.set_vtol_client(self.vtol_MCcmd)
            if response.success:
                pass
                #rospy.loginfo("VTOL transition successful(MC)")
            else:
                rospy.logerr("VTOL transition failed(MC)")
    ...
    def reset(self):
        ...
        init_action = np.array([[0,0,10],[0,0,0*(math.pi/180)]])  # [x, y, z],[pitch, roll, yaw]
